namespace AFV2
{
    ...
    public class CharacterCombatDecision : MonoBehaviour
    {
        ...
        private (WeaponInstance rightHandWeapon, WeaponInstance leftHandWeapon) GetActiveWeapons()
        {
            characterApi.characterEquipment.characterWeapons.TryGetActiveRightWeapon(out WeaponInstance rightHandWeapon);
            characterApi.characterEquipment.characterWeapons.TryGetActiveLeftWeapon(out WeaponInstance leftHandWeapon);
        ...
        {
            if (!characterApi.characterEquipment.characterWeapons.TryGetActiveRightWeapon(out WeaponInstance rightWeapon))
                return false;
        ...
        {
            if (!characterApi.characterEquipment.characterWeapons.TryGetActiveLeftWeapon(out WeaponInstance leftWeapon))
                return false;

            return CanLightAttack(leftWeapon);
        }

        bool CanLightAttack(WeaponInstance weapon) => characterApi.characterStamina.HasEnoughStamina(weapon.equippedWeapon.lightAttackStaminaCost);

        #endregion

        #region  Air Combat

        private (List<string> availableAttacks, float staminaCost) GetAttackDetails(WeaponInstance weapon, CombatDecision combatDecision)
        {
