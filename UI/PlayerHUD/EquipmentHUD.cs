namespace AF
{
    ...
    public class EquipmentHUD : MonoBehaviour
    {
        ...
        private void UpdateSlot(IMGUIContainer slot, ItemInstance itemInstance, Texture2D unequippedSlot, Texture2D alternativeUnequippedSlot = null)
        {
            slot.style.backgroundImage = itemInstance.Exists()
                ? new StyleBackground(itemInstance.GetItem().sprite)
        ...
        private void UpdateArrowsAndQuickItemName()
        {
            ...
            quickItemName.text = consumable != null
                ? $"{consumable.GetItem()?.GetName()} ({inventoryDatabase.GetItemAmount(consumable.GetItem())})"
                : "";
            ...
            arrowsLabel.text = equipmentDatabase.IsBowEquipped() && equipmentDatabase.GetCurrentArrow() != null
                ? equipmentDatabase.GetCurrentArrow().GetItem()?.GetName() + " (" + inventoryDatabase.GetItemAmount(equipmentDatabase.GetCurrentArrow().GetItem()) + ")"
                : "";
        }
