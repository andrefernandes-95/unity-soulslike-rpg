...
namespace AF
{
    ...
    public class FavoriteItemsManager : MonoBehaviour
    {
        ...
        public void OnConsumableUse()
        {
            ...
            ConsumableInstance consumableInstance = equipmentDatabase.GetCurrentConsumable();
            ...
            /*
                        int itemAmount = inventoryDatabase.GetItemAmount(consumableInstance.GetItem());

                        if (itemAmount <= 1 && (!consumableInstance.GetItem()?.isRenewable ?? false))
                        {
                            equipmentDatabase.UnequipConsumable(equipmentDatabase.currentConsumableIndex);
                        }*/

            playerManager.playerInventory.PrepareItemForConsuming(consumableInstance.GetItem());
