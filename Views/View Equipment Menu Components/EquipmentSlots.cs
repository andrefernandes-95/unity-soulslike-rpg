namespace AF.UI.EquipmentMenu
{
    ...
    public class EquipmentSlots : MonoBehaviour
    {
        ...
        {
            Dictionary<Button, Func<ShieldInstance>> buttonDictionary = new()
            {
        ...
        void OnSlotFocus(string activeSlotMenuLabel, ItemInstance itemInstance)
        {
            ...
            {
                displayName = itemInstance.GetItem().GetName();
        ...
        void SetBackgroundImage(VisualElement button, ItemInstance[] items, int index, Texture2D unequippedTexture)
        {
            if (index < items.Length)
            {
                ...
                {
                    button.style.backgroundImage = new StyleBackground(items[index].GetItem().sprite);
                }
                else
                {
        ...
        {
            if (index < equipmentDatabase.shields.Length && equipmentDatabase.shields[index].Exists())
            {
                button.style.backgroundImage = new StyleBackground(equipmentDatabase.shields[index].GetItem().sprite);
                return;
            }

            if (index < equipmentDatabase.secondaryWeapons.Length && equipmentDatabase.secondaryWeapons[index].Exists())
            {
                button.style.backgroundImage = new StyleBackground(equipmentDatabase.secondaryWeapons[index].GetItem().sprite);
                return;
            }
